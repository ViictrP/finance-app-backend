import deleteTransactionUsecase from '../../../../src/core/usecases/delete-transaction.usecase';
...
describe('deleteTransactionUseCase', () => {
  ...
  it('Should delete transaction with success', async () => {
    ...
    expect(repository.deleteTransaction).toHaveBeenCalledWith(
      transaction,
      false
    );
  });
  ...
  it('Should throw error if transaction doesnt exist', async () => {
    ...
    await expect(
      deleteTransactionUsecase('test', false, repository as any)
    ).rejects.toThrowError(new Error('transaction not found for id test'));
  });
});
